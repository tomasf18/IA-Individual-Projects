...
constraints = { (v1,v2):(lambda v1,x1,v2,x2: x1!=x2) # (variavel1, valor1, variavel2, valor2)
                for v1 in variables for v2 in variables if v1!=v2 } # ATENCAO: inclui ambas as direcoes para cada par!!!!!!!!!!!!!!!!
...
# Define variables
variables = ['S', 'E', 'N', 'D', 'M', 'O', 'R', 'Y', 'X1', 'X2']

# Define domains
digits = list(range(10))
domains = {v: digits for v in variables if v not in ['S', 'M']}
domains['S'] = list(range(1, 10))  # S cannot be 0
domains['M'] = list(range(1, 10))  # M cannot be 0
domains['X1'] = [0, 1]  # Carry-over constraints
domains['X2'] = [0, 1]

# Constraints
constraints = {
    (v1, v2): (lambda v1, x1, v2, x2: x1 != x2)  # All variables must be different
    for v1 in variables for v2 in variables if v1 != v2
}

# Higher-order constraints for the equation
handle_ho_constraint(domains, constraints, ['D', 'E', 'Y', 'X1'], lambda t: t[0] + t[1] == t[2] + 10 * t[3])
handle_ho_constraint(domains, constraints, ['N', 'R', 'E', 'X1'], lambda t: t[0] + t[1] + t[3] == t[2] + 10 * t[3])
handle_ho_constraint(domains, constraints, ['E', 'O', 'N', 'X2'], lambda t: t[0] + t[1] + t[3] == t[2] + 10 * t[3])
handle_ho_constraint(domains, constraints, ['S', 'M', 'O', 'X2'], lambda t: t[0] + t[1] == t[2] + 10 * t[3])
...
for solution in solutions:
    print({v: solution[v] for v in variables})
...
#           + M O R E
#         ----------       ( much simplified formulation
#           M O N E Y        using function handle_ho_constraint() )

print("\n-- ## Ex. 5) --------------------------------")
 
variables = ['S', 'E', 'N', 'D', 'M', 'O', 'R', 'Y']

# Define digits for the puzzle
digits = list(range(10))

# Define domains
domains = {v: digits for v in variables}
domains.update({v: [0, 1] for v in ['X1', 'X2', 'X3', 'X4', 'M']})
...
# Higher-order constraints
handle_ho_constraint(domains, constraints, ['D', 'E', 'Y', 'X1'], lambda t: (t[0] + t[1]) % 10 == t[2] and (t[0] + t[1]) // 10 == t[3])
handle_ho_constraint(domains, constraints, ['N', 'R', 'E', 'X1', 'X2'], lambda t: (t[0] + t[1] + t[3]) % 10 == t[2] and (t[0] + t[1] + t[3]) // 10 == t[4])
handle_ho_constraint(domains, constraints, ['E', 'O', 'N', 'X2', 'X3'], lambda t: (t[0] + t[1] + t[3]) % 10 == t[2] and (t[0] + t[1] + t[3]) // 10 == t[4])
handle_ho_constraint(domains, constraints, ['S', 'M', 'O', 'X3', 'X4'], lambda t: (t[0] + t[1] + t[3]) % 10 == t[2] and (t[0] + t[1] + t[3]) // 10 == t[4])
